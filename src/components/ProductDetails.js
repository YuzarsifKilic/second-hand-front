...
export default function ProductDetails() {
    ...
    return (
        <div className={"mt-10"}>
            <div>
            <div className={"ml-56 grid grid-cols-2 gap-4"}>
                <div>
                    <img src={product.photo_url} style={{height: "500px", width: "500px"}} className={"rounded-xl"}/>
                </div>
                <div className={"mt-1"}>
                    <p>
                    ...
                    <p>
                        Model: {product.brand_model}
                    </p>
                    <p>
                        Fiyat: {product.price}
                    </p>
                    <p>
                        Model Yılı: {product.model_year}
                    </p>
                    <p>
                        İşlemci Markası: {product.cpu_brand}
                    </p>
                    <p>
                        İşlemci Modeli: {product.cpu_brand_model}
                    </p>
                    <p>
                        İşlemci Hızı: {product.cpu_speed}
                    </p>
                    <p>
                        Ekran Kartı Markası: {product.gpu_brand}
                    </p>
                    <p>
                        Ekran Kartı Modeli: {product.gpu_brand_model}
                    </p>
                    <p>
                        Ekran Kartı (GB): {product.gpu_size}
                    </p>
                    <p>
                        Ram (GB): {product.ram_size}
                    </p>
                    <p>
                        Ekran Kartı: {product.gpu_size}
                    </p>
                    <p>
                        Ekran Boyutu: {product.screen_size}
                    </p>
                    <p>
                        Ekran Çözünürlüğü: {product.resolution}
                    </p>
                    <NavLink to={"/message"}>
                    <button
                        className={"rounded-xl bg-sky-800 p-2 text-white mt-5 ml-48 w-36"}
                    >
                        Anlaş
                    </button>
                    </NavLink>
